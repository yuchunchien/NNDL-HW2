...
class myMLP(object):
    def __init__(self, rng, input, n_in, n_hidden, n_out, HLAmt, activation):
        # Since we are dealing with a one hidden layer MLP, this will translate
        # into a HiddenLayer with a tanh activation function connected to the
        # LogisticRegression layer; the activation function can be replaced by
        # sigmoid or any other nonlinear function
        ...
        for HLNum in range(HLAmt):
            if HLNum == 0:
                self.HLs.append(HiddenLayer(
                    rng=rng,
                    input=input,
                    n_in=n_in,
                    n_out=n_hidden,
        ...
        for HLNum in range(HLAmt):
            self.L1 += abs(self.HLs[HLNum].W).sum()
        ...
        for HLNum in range(HLAmt):
            self.L2_sqr += (self.HLs[HLNum].W ** 2).sum()
        ...
        for HLNum in range(HLAmt):
            self.params = self.HLs[HLNum].params + self.params
...
def test_mlp(learning_rate=0.01, L1_reg=0.00, L2_reg=0.0001, n_epochs=400,
             batch_size=20, n_hidden=250, verbose=False, HLAmt=2,              
    ...
    return (1.0-test_score) * 100.

if __name__ == '__main__':
    test_mlp(n_hidden=250, HLAmt=2, activation=T.nnet.softmax, verbose=True)
